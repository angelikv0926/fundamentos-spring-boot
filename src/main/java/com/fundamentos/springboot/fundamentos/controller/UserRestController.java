...
public class UserRestController {
    ...
    @DeleteMapping("/{id}")
    ResponseEntity deleteUser(@PathVariable BigDecimal id){
        deleteUser.remove(id);
        return new ResponseEntity(HttpStatus.NO_CONTENT);
    }

    @PutMapping("{id}")
    ResponseEntity<User> updateUser(@RequestBody User newUser, @PathVariable BigDecimal id){
        return new ResponseEntity<>(updateUser.update(newUser, id), HttpStatus.OK);
    }

    @GetMapping("/pag")
    List<User> getUserPag(@RequestParam int page, @RequestParam int size){
